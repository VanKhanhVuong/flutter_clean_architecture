...
import 'package:fluttercleanarchitecture/features/auth/presentation/verify_delete_account/controller/resend_delete_account_controller.dart';
...
import 'package:fluttercleanarchitecture/features/auth/presentation/verify_delete_account/ui/widgets/submit_verify_delete_button.dart';
...
class VerifyDeleteAccountFormList extends ConsumerStatefulWidget {
  final String email;
  const VerifyDeleteAccountFormList({required this.email, super.key});

  @override
  ConsumerState<ConsumerStatefulWidget> createState() =>
      _VerifyDeleteFormListState();
}

class _VerifyDeleteFormListState
    extends ConsumerState<VerifyDeleteAccountFormList> {
  // form key
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                Column(
                  children: [
                    SubmitVerifyDeleteButton(onPressed: _onSubmit),
  ...
  void _listenerResendCode() {
    ref.listen(
      resendDeleteAccountControllerProvider.select(
    ...
    ref.listen(
      resendDeleteAccountControllerProvider.select((value) => value.isSuccess),
      (_, next) {
        if (next != null && next) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
  ...
  void _listenerVerifyAccount() {
    //   // listen for error
    ref.listen(
      verifyDeleteAccountControllerProvider.select(
        (value) => value.errorMessage,
      ),
      (_, next) {
        if (next != null) {
    ...
    // listen for success
    ref.listen(
      verifyDeleteAccountControllerProvider.select((value) => value.isSuccess),
      (_, next) {
        if (next != null && next) {
          showDialog(
            context: context,
            ...
            builder: (context) {
              return AlertDialog(
                title: Text('Verify Delete Account Successful'.hardcoded),
                content: Text(
  ...
  void _onResend() {
    ...
    // set form data
    ref
        .read(resendDeleteAccountControllerProvider.notifier)
  ...
  void _onSubmit() {
    ...
    if (isValid) {
      ...
      // set form data
      ref
          .read(verifyDeleteAccountControllerProvider.notifier)
          .setFormData(formData);
      // sign up - call api
      ref
          .read(verifyDeleteAccountControllerProvider.notifier)
